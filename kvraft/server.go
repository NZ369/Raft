...
type Result struct {
	...
	ClientId  int64
	RequestId int64
	// Because the term of index position log is inconsistent, it means that the leader has been
	// changed or ignored bool or because the req id expired, the log was skipped for execution
	WrongLeader bool
	Err         Err
	Value       string
}
...
type KVServer struct {
	...
	// ack corresponds to the maximum seq number of each client's current processing request (used for deduplication)
	resultCh map[int]chan Result // log index to result of applying that entry
}

//
// Function will attempt to the entry to respective raft servers' logs and return result.
// The result will be valid if raft servers commit added entry prior to timeout.
//
...
//
// Conduct the current operation on the database and return result.
// Each server should execute Op commands as Raft commits them, as in-
// as they appear on the applyChan. An RPC handler notices when Raft
// commits its Op, and then reply to the RPC.
//
...
func (kv *KVServer) Run() {
	for {
		...
		// commands to the Raft log using KVStart().
		msg := <-kv.applyCh
		kv.mu.Lock()
		if msg.UseSnapshot {
