...
func (kv *ShardKV) applyReconfigure(args Op, result *Result) {
	...
	if args.Config.Num == kv.config.Num+1 {
		...
		// Combine ack maps from other args with this server's ack map
...
//
// Function determines if the key is handled correctly by the replica group
//
...
//
// Function copies shards required in args and ack to reply when the server is ready.
//
...
//
// Function retrieves shards in args and ack from replica group specified by gid
//
...
//
// Function will collect shards from other replica groups in order to process reconfiguration and
// recreate the log entry to be appended into the raft log.
//
	...
	for gid, shardIds := range transferShards {
		...
		go func(gid int, args TransferShardArgs, reply TransferShardReply) {
			...
			if kv.sendTransferShard(gid, &args, &reply) {
				ackMu.Lock()
				// Will only copy shards requested from that replica group to reconfigure args
				for _, shardId := range args.ShardIds {
				...
				// Will merge ack map from that replica group to reconfigure args
...
//
// The function builds a map from gid to shard ids in order to send a request from replica group specified by gid.
//
...
//
// Function checks if this server is the leader of the replica group
// the function also checks the latest configuration periodically and
// attempt to update configuration when necessary.
//
...
//
// Function determines if the server is ready, if so it will generate an entry
// that will delete the shard in args and then attempt to append it onto raft
// servers' log.
//
...
//
// Function will request replica group specified by gid in last config in order to
// delete the shard no longer required.
//
